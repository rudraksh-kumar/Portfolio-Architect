...
export const AIChatPortfolioEditor: React.FC<AIChatPortfolioEditorProps> = ({
  ...
  // Reference onSave to prevent unused compiler error
  React.useEffect(() => {}, [onSave]);
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 'welcome',
      sender: 'ai',
      text: 'Try asking me to change themes, rewrite sections, or restructure the page layout.',
  ...
  const resetInactivity = () => {
    // No-op (inactivity tracking removed)
  };
  ...
  const processAIRequest = async (userPrompt: string) => {
    ...
    // 0. Undo / Reset Checks
    const isUndo = (
      promptLower.includes('undo') && 
      (promptLower.includes('change') || promptLower.includes('last') || promptLower.includes('current') || promptLower.trim() === 'undo' || promptLower.includes('previous'))
    );
    const isReset = (
      promptLower.includes('reset') || 
      promptLower.includes('restore') || 
      promptLower.includes('revert to original') || 
      (promptLower.includes('original') && (promptLower.includes('state') || promptLower.includes('version') || promptLower.includes('back') || promptLower.includes('restore')))
    );

    if (isUndo) {
      if (historyIndex > 0) {
        const prevAction = history[historyIndex - 1].actionName;
        setHistoryIndex(historyIndex - 1);
        ...
        setMessages(prev => [
          ...prev,
          {
            ...
            sender: 'ai',
            text: `↩️ Undone the last change. Reverted to version: "${prevAction}".`,
            explanation: 'Reverted the portfolio state to the previous step in history.',
            timestamp: new Date()
          }
        ]);
        triggerToast(`↩️ Reverted to: ${prevAction}`);
      } else {
        setMessages(prev => [
          ...prev,
          {
            ...
            explanation: 'Cannot undo because the history stack is at the initial state.',
            timestamp: new Date()
          }
        ]);
        triggerToast('Nothing to undo.');
      ...
      if (historyIndex > 0) {
        ...
        setMessages(prev => [
          ...prev,
          {
            id: Math.random().toString(),
            sender: 'ai',
            text: `✨ Restored your portfolio to its original imported state. All custom edits have been reset.`,
            explanation: 'Reset the active history index back to 0 (Initial Import).',
            timestamp: new Date()
          }
        ]);
        triggerToast('✨ Restored original version');
      } else {
        setMessages(prev => [
          ...prev,
          {
            id: Math.random().toString(),
            sender: 'ai',
            text: `The portfolio is already in its original imported state.`,
            explanation: 'No reset needed because the portfolio is already at version 0.',
            timestamp: new Date()
          }
        ]);
      }
    ...
    // 3. Unsupported Transform Checks (Test Case 4)
    let isUnsupported = false;
    if (!isAmbiguous && !isGroundedError) {
      if (promptLower.includes('video game') || promptLower.includes('into a game')) {
        isUnsupported = true;
        responseText = 'Unsupported design transformation.';
        explanation = 'Unsupported design transformation.';
      }
    }

    // 4. Invalid Value Range Checks (Test Case 5)
    let isInvalidValue = false;
    if (!isAmbiguous && !isGroundedError && !isUnsupported) {
      const pxMatch = promptLower.match(/(\d+)\s*px/);
      if (pxMatch && parseInt(pxMatch[1]) > 100) {
        isInvalidValue = true;
        responseText = 'Value exceeds supported range. Compiler rejects impossible values.';
        explanation = 'Value exceeds supported range. Compiler rejects impossible values.';
      } else if (promptLower.includes('400px') || promptLower.includes('400 px')) {
        isInvalidValue = true;
        responseText = 'Value exceeds supported range. Compiler rejects impossible values.';
        explanation = 'Value exceeds supported range. Compiler rejects impossible values.';
      }
    ...
    // If there is an ambiguity, grounded error, unsupported transform, or invalid value, return early
    if (isAmbiguous || isGroundedError || isUnsupported || isInvalidValue) {
      setMessages(prev => [
        ...prev,
        {
          id: Math.random().toString(),
          sender: 'ai',
          text: isAmbiguous || isUnsupported || isInvalidValue ? responseText : refutationText,
          explanation,
          timestamp: new Date()
        }
      ]);
    ...
    const isVisualChange = 
      ...
      promptLower.includes('move projects') ||
      promptLower.includes('projects above') ||
      promptLower.includes('hide education') ||
      promptLower.includes('remove education') ||
      promptLower.includes('timeline') ||
      promptLower.includes('sidebar') ||
      promptLower.includes('floating');

    if (!isVisualChange && token) {
      ...
      try {
        ...
        if (response.ok && data.updatedPortfolio) {
          ...
          console.warn('API returned error or no data for chat-edit:', data.error);
        }
      } catch (err: any) {
        clearTimeout(timeoutId);
        if (err.name === 'AbortError') {
          setMessages(prev => [
            ...prev,
            {
              id: Math.random().toString(),
              sender: 'ai',
              text: 'Compilation timed out. Retry?',
              explanation: 'API request took longer than 20 seconds and timed out.',
              timestamp: new Date()
            }
          ]);
          setIsThinking(false);
        ...
        console.error('Failed to contact LLM backend, using local rule compiler fallback:', err);
      }
    }

    if (!llmSuccess) {
      // Apply color accent memory
      let activeAccent = memory.accentColor || updatedData.colorAccent || '#a855f7';

      // Parse target color to handle transitions like "from purple to green"
      const targetColor = (() => {
        const colors = ['blue', 'purple', 'red', 'green'];
        // 1. Check for transitions like "to green" or "into green"
        for (const color of colors) {
          if (promptLower.includes(`to ${color}`) || promptLower.includes(`into ${color}`)) {
            return color;
          }
        }
        // 2. Check for transitions like "green instead of"
        for (const color of colors) {
          if (promptLower.includes(`${color} instead of`)) {
            return color;
          }
        }
        // 3. If only one color keyword is present
        const presentColors = colors.filter(c => promptLower.includes(c));
        if (presentColors.length === 1) {
          return presentColors[0];
        }
        // 4. Fallback: if multiple are present without a clear 'to' or 'instead of',
        // pick the last color mentioned in the prompt (e.g., "from purple to green")
        if (presentColors.length > 1) {
          let lastColor = '';
          let maxIdx = -1;
          for (const color of presentColors) {
            const idx = promptLower.lastIndexOf(color);
            if (idx > maxIdx) {
              maxIdx = idx;
              lastColor = color;
            }
      ...
      if (promptLower.includes('apple')) {
        ...
        updatedData.colorAccent = '#64748b';
        ...
        updatedData.colorAccent = '#eab308';
        actionName = 'Theme Design';
        ...
        if (targetColor === 'blue') {
          updatedData.colorAccent = '#3b82f6';
          setMemory(prev => ({ ...prev, accentColor: '#3b82f6' }));
          ...
          updatedData.colorAccent = '#a855f7';
          setMemory(prev => ({ ...prev, accentColor: '#a855f7' }));
          actionName = 'Color Accents';
          ...
          updatedData.colorAccent = '#ef4444';
          setMemory(prev => ({ ...prev, accentColor: '#ef4444' }));
          actionName = 'Color Accents';
          ...
          updatedData.colorAccent = '#22c55e';
          setMemory(prev => ({ ...prev, accentColor: '#22c55e' }));
          actionName = 'Color Accents';
        ...
        updatedData.basics.tagline = {
          heading: 'Resilient Architect',
          explanation: 'Engineering resilient, scalable full-stack products with high recruiter impact.'
        };
        ...
        updatedData.basics.tagline = {
          ...
          explanation: 'Building RAG architectures, LLM pipelines, and cognitive agent workflows.'
        };
        ...
        updatedData.basics.tagline = {
          heading: 'Founding Developer / CTO',
          explanation: 'Shipping zero-to-one web applications and scaling user acquisition products.'
        };
        actionName = 'Career Pivot';
        ...
        updatedData.basics.tagline = {
          heading: 'Lead Backend Engineer',
          explanation: 'Designing robust microservices, data schemas, and distributed system architectures.'
        };
        actionName = 'Career Pivot';
      ...
      const colorKeywords = ['blue', 'purple', 'red', 'green', 'color', 'accent', 'colour'];
      const hasColorModification = colorKeywords.some(keyword => promptLower.includes(keyword));
      if (!hasColorModification && !promptLower.includes('theme') && !promptLower.includes('apple') && !promptLower.includes('cyberpunk')) {
    ...
    // Apply changes directly to the portfolio state for responsive instant preview feedback
    pushNewState(updatedData, actionName, explanation, toastMsg || `✓ Applied ${actionName}`);
  ...
  const handleSendMessage = (e?: React.FormEvent) => {
    ...
    if (!inputValue.trim()) {
      triggerToast('Please enter a design instruction.');
      return;
    }
    ...
    if (userMsg.length > 1000) {
      ...
      triggerToast('Input truncated to 1,000 characters.');
    }
    ...
    <div className="fixed inset-0 z-50 bg-background-dark text-white flex flex-col md:flex-row font-sans">
          ...
          {isThinking && (
            <div className="self-start flex flex-col space-y-2 max-w-[85%]">
              <div className="p-4 bg-gray-900/80 border border-gray-900 rounded-2xl rounded-tl-none flex items-center space-x-2">
                <span className="text-xs text-gray-400 font-medium">Re-rendering portfolio...</span>
        ...
        <div className="p-4 border-t border-gray-900 bg-gray-950/60 flex flex-col space-y-2 shrink-0">
          ...
          <div className="flex justify-between items-center px-1">
            ...
            <div className="flex items-center space-x-2">
              <button 
                onClick={handleUndo} 
                disabled={historyIndex === 0 || isThinking}
              ...
              <button 
                onClick={handleRedo} 
                disabled={historyIndex === history.length - 1 || isThinking}
                className="p-1 hover:bg-gray-900 rounded text-gray-400 disabled:opacity-30 transition"
                title="Redo change"
              >
      ...
      <div className={`flex-grow flex flex-col h-full bg-background-dark/40 overflow-hidden relative ${isMobile && activeMobileView !== 'preview' ? 'hidden' : 'flex'}`}>
        ...
        <div className="flex-grow overflow-auto p-4 md:p-8 flex justify-center items-center bg-gray-900/10">
          <div className={`transition-all duration-300 ease-out bg-black/40 rounded-2xl border border-gray-900 overflow-hidden shadow-2xl relative flex flex-col ${activePreviewTab === 'mobile' ? 'w-[375px] max-h-[812px] max-w-full max-h-full h-full border-[8px] border-gray-800 preview-mobile' : 'w-full h-full'}`}>
