  ...
  return (
    ...
    >
          ...
          <div className={`${isPreview ? 'flex' : 'flex md:hidden'} items-center gap-3`}>
            ...
            {isNavDropdownOpen && (
              <div className="absolute top-[115%] left-1/2 -translate-x-1/2 w-48 bg-zinc-950 border border-white/5 rounded-2xl shadow-2xl p-1.5 space-y-1 z-50 text-[10px] font-bold tracking-wider font-mono">
                ...
                <a href="#copilot" onClick={() => setIsNavDropdownOpen(false)} className="block px-4 py-2 bg-purple-950/40 hover:bg-purple-900/30 text-purple-400 hover:text-purple-300 rounded-xl transition">RECRUITER COPILOT</a>
              </div>
            )}
      ...
      <footer className="max-w-6xl w-full mx-auto px-6 py-6 border-t border-white/5 text-center text-gray-500 text-xs">
        <p>© 2026 {basics.name}. Generated by Portfolio Architect.</p>
      </footer>
    </div>
  );
};
