...
export class PortfolioController {
    ...
    static async generate(req, res, next) {
        try {
            ...
            const generatedProfile = await GroqService.generatePortfolioData(resumeText, linkedinText || '');
            ...
            if (githubUrl) {
                ...
                if (githubUsername) {
                    try {
                        ...
                        generatedProfile.githubData = githubData;
                    }
                    catch (gitErr) {
            ...
            if (global.responseCache) {
                const cache = global.responseCache;
     ...
     */
    static async chatCopilot(req, res, next) {
        try {
            const { slug } = req.params;
            ...
            const profileJson = JSON.parse(portfolio.profileData);
            ...
            let responseText = history && history.length > 0 ? null : global.responseCache[cacheKey];
            if (responseText) {
                console.log(`[Cache Hit] Serving cached response for key: "${cacheKey}"`);
            }
            else {
                // Request Copilot Response
                responseText = await GroqService.answerCopilotQuery(profileJson, message, history || []);
                // Only save to cache if history is empty (one-off query)
                if (!history || history.length === 0) {
                    global.responseCache[cacheKey] = responseText;
                }
            }
            ...
            if (recruiterMatch && recruiterMatch[1]) {
                ...
                if (matched)
                    inferredOrg = matched[1].charAt(0).toUpperCase() + matched[1].slice(1).toLowerCase();
            }
